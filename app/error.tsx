...
export default function ErrorPage({ error, reset }: ErrorProps) {
  useEffect(() => {
    // Log error to an analytics reporting service if needed
      ...
      <div className="relative z-10 max-w-md w-full text-center space-y-6">
        ...
        >
          ...
          <p className="text-sm text-slate-500 leading-relaxed max-w-sm mx-auto">
            Si è verificato un errore inaspettato durante il caricamento di questa sezione. Riprova o torna in Home.
          </p>
          {error.digest && (
        ...
        >
          <button
            onClick={() => reset()}
            className="w-full sm:w-auto inline-flex h-10 items-center justify-center gap-1.5 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 px-5 text-xs font-semibold text-slate-700 shadow-sm transition-colors"
          >
            <RotateCcw size={14} />
            Riprova
          </button>
